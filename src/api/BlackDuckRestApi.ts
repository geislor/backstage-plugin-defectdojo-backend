...
export class defectdojoRestApi {
    ...
    try {
      const auth = await fetch(`${this.host}/tokens/authenticate`, {
        ...
        headers: {
          ...
          Accept: "application/vnd.defectdojosoftware.user-4+json",
    ...
    const projects = await fetch(
      `${this.host}/projects?limit=999&q=name:${name}`,
      {
        ...
        headers: {
          ...
          Accept: "application/vnd.defectdojosoftware.project-detail-4+json",
          "Content-Type": "application/json",
        },
      }
    );
  ...
  public async getVersions(
    ...
    const versions = await fetch(
      `${projectUrl}/versions?limit=999&q=versionName:${versionName}`,
      {
        ...
        headers: {
          Authorization: `Bearer ${this._bearer}`,
          Accept: "application/vnd.defectdojosoftware.project-detail-5+json",
          "Content-Type": "application/json",
        },
      }
    );
  ...
  public async getVulnerabilities(projectName: string, projectVersion: string) {
    ...
    const vulns: any = await fetch(vuln_url, {
      ...
      headers: {
        Authorization: `Bearer ${this._bearer}`,
        Accept: "application/vnd.defectdojosoftware.bill-of-materials-6+json",
        "Content-Type": "application/json",
      },
    });
    this.logger.verbose(
